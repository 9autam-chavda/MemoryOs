      ...
      <div className="grid w-full max-w-5xl overflow-hidden rounded-[32px] border border-zinc-800/80 bg-zinc-950/80 shadow-[0_30px_90px_rgba(0,0,0,0.45)] lg:grid-cols-[1.05fr_0.95fr]">
        <div className="flex flex-col justify-between border-b border-zinc-800/80 bg-zinc-900/70 p-8 sm:p-10 lg:border-b-0 lg:border-r">
          <div>
            ...
            <h1 className="mt-6 text-3xl font-semibold tracking-tight text-zinc-50 sm:text-4xl">Build a searchable memory workspace.</h1>
            <p className="mt-3 max-w-md text-sm leading-7 text-zinc-400 sm:text-base">
              Create an account and start turning uploaded files into an intelligent personal knowledge base.
            </p>
          </div>

          <div className="mt-10 rounded-2xl border border-zinc-800 bg-zinc-950/70 p-4">
            <div className="flex items-center gap-2 text-sm font-medium text-zinc-200">
              <Sparkles size={16} className="text-blue-300" /> AI-first knowledge management
            </div>
            <p className="mt-2 text-sm leading-6 text-zinc-500">
              Organize everything from design docs to meeting recordings into one calm, indexed workspace.
            </p>
          </div>
        </div>
        ...
        <div className="p-8 sm:p-10">
          <div className="mb-8">
            <p className="text-sm font-medium uppercase tracking-[0.24em] text-blue-300">Create account</p>
            <h2 className="mt-2 text-2xl font-semibold text-zinc-50">Start with MemoryOS</h2>
          </div>

          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="mb-2 block text-sm text-zinc-400" htmlFor="name">Name</label>
              <input id="name" type="text" name="name" placeholder="Alex Morgan" value={formData.name} onChange={handleChange} className="w-full rounded-2xl border border-zinc-800 bg-zinc-900/80 px-4 py-3 text-sm text-zinc-100 outline-none transition focus:border-blue-500/40" />
            </div>
            <div>
          ...
          <div className="mt-6 text-sm text-zinc-500">
            Already have an account? <Link to="/" className="font-medium text-blue-300 transition hover:text-blue-200">Sign in</Link>
          </div>
        </div>
      </div>
    </div>
